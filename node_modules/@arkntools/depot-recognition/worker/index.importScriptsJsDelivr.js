self.importScripts('https://fastly.jsdelivr.net/npm/lodash@4.17.21/lodash.min.js', 'https://fastly.jsdelivr.net/npm/simple-statistics@7.8.3/dist/simple-statistics.min.js', 'https://fastly.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js', 'https://fastly.jsdelivr.net/npm/@arkntools/scripts@1.1.1/dist/ocrad.js', 'https://fastly.jsdelivr.net/npm/@arkntools/scripts@1.2.0/dist/jimp4worker.js');
...
const getRangeEnd = ({ start, length }) => start + length;
const inRange = (x, { start, length }) => start <= x && x < start + length;
const findRangeIndex = (x, ranges) => _.findIndex(ranges, range => inRange(x, range));
const getRanges = (arr) => _.transform(arr, (a, inRange, x) => {
    if (!a.length) {
        if (inRange)
...
const removeRangesNoise = (ranges, size = 1) => _.remove(ranges, ({ length }) => length <= size);
...
const itemDetection = (origImg, isDebug = false) => {
    ...
    const scale = (() => {
        ...
        if (w >= h && w > ORIG_MAX_WIDTH)
            img.resize(ORIG_MAX_WIDTH, Jimp.AUTO);
        else if (h > w && h > ORIG_MAX_HEIGHT)
            img.resize(Jimp.AUTO, ORIG_MAX_HEIGHT);
        else
            return 1;
    ...
    const yRanges = getRangesBy(yWhite.map(v => v > yRangeMinLine), ({ length }, minLen) => length > minLen, ranges => _.maxBy(ranges, 'length').length * 0.8);
    let itemWidth = _.min(_.map(yRanges, 'length'));
    ...
    edgeImg.scan(0, 0, width, height, function (x, y, idx) {
        ...
        if (yRangeIndex !== -1)
            xWhites[yRangeIndex][x] += this.bitmap.data[idx];
    });
    const xsRanges = xWhites.map(xWhite => getRangesBy(xWhite.map(v => v > 0), ({ start, length }) => start > 0 && start + length < width && itemWidth * 0.9 < length && length < itemWidth * 1.2));
    const xRangeMinLength = 0.05 * itemWidth;
    const xItemWidths = _.map(_.flatten(xsRanges).filter(({ start, length }) => start !== 0 && start + length !== width && length < itemWidth && length > xRangeMinLength), 'length');
    if (xItemWidths.length) {
        itemWidth = _.min(xItemWidths);
    }
    const itemsRange = _.flatten(xsRanges.map((xRanges, y) => {
    ...
    itemWidth =
        _.min(_.flatMap(itemsRange, ({ xRange, yRange }) => [xRange.length, yRange.length])) || itemWidth;
    ...
    const positions = _.flatMap(_.uniqBy(_.flatten(xPoss), 'pos.x'), xPos => _.uniqBy(_.flatten(yPoss), 'pos.y').map(yPos => _.merge({
        debug: {
...
var ImageData$1 = typeof ImageData === 'undefined'
    ? ImageDataPolyfill
    : ImageData;
...
const splitNumbers = ({ splittedImgs, itemWidth, simResults, IMG_SL, }) => {
    ...
    return splittedImgs.map((splittedImg, i) => {
        ...
        _.remove(numImgBlackRanges, ({ start, length }, j) => {
            ...
            if (yBlack[0] || _.last(yBlack))
                return true;
            if (_.sum(_.map(yRanges, 'length')) < numImgH * 0.5)
                return true;
            return false;
        });
...
const getSim = (input, imgMap, order) => {
    if (!order.length)
        return null;
    const diffs = _.sortBy(order.map(id => [id, Jimp.diff(input, imgMap.get(id), 0.2).percent]), 1);
...
class DeportRecognizer {
    ...
    async recognize(file, onProgress = () => { }) {
        ...
        return {
            data: _.merge(positions, simResults.map(sim => ({ sim })), numResults.map(num => ({ num }))),
            debug: await Promise.all(debugImgs.map(jimp2base64)),
        };
    }
}

export { DeportRecognizer };
