...
const findRangeIndex = (x, ranges) => findIndex(ranges, range => inRange(x, range));
const getRanges = (arr) => transform(arr, (a, inRange, x) => {
    if (!a.length) {
        if (inRange)
...
const removeRangesNoise = (ranges, size = 1) => remove(ranges, ({ length }) => length <= size);
...
const itemDetection = (origImg, isDebug = false) => {
    ...
    const yRanges = getRangesBy(yWhite.map(v => v > yRangeMinLine), ({ length }, minLen) => length > minLen, ranges => maxBy(ranges, 'length').length * 0.8);
    let itemWidth = min(map(yRanges, 'length'));
    ...
    const xItemWidths = map(flatten(xsRanges).filter(({ start, length }) => start !== 0 && start + length !== width && length < itemWidth && length > xRangeMinLength), 'length');
    if (xItemWidths.length) {
        itemWidth = min(xItemWidths);
    }
    const itemsRange = flatten(xsRanges.map((xRanges, y) => {
    ...
    const positions = flatMap(uniqBy(flatten(xPoss), 'pos.x'), xPos => uniqBy(flatten(yPoss), 'pos.y').map(yPos => merge({
        debug: {
